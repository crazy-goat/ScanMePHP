...
/**
 * Codewords to a Data Matrix symbol: size, error correction, placement, frame.
 *
 * Everything here is downstream of encodation and knows nothing about it. That
 * separation is the point: GS1 Data Matrix differs from a plain one only in how
 * the bytes become codewords — one FNC1 codeword in front and one wherever a
 * separator goes — and in nothing after. Sharing this means the two cannot
 * drift apart in size selection, block interleaving or the finder frame, none
 * of which either symbology gets a say in.
 *
 * @internal
 */
final class SymbolBuilder
{
    ...
    /**
     * @param list<int> $codewords Data codewords, before padding
     * @param array<string, int|string|bool> $metadata Merged over the size figures
     */

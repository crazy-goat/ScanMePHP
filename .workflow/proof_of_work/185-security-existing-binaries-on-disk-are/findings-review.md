...
## Round 5 (review, commit 39a8b13 — verdict: CLEAN)

Final gate before PR. Re-check of F-1…F-11 with evidence (code inspection + full runs + red mutation probe for F-11):

| # | severity | round-5 status | evidence |
|---|----------|----------------|----------|
| F-1 | medium | **fixed** | Seam `Plugin::createDownloader()` protected (Plugin.php:262); ext mismatch test (PluginTest.php:108-143) green in focused run (7/7, 30 assertions); `39a8b13` touches no `src/` file. |
| F-2 | low | **fixed** (FFI mirror now pinned via F-11) | `is_file()` at Plugin.php:148 (ext) and :221 (FFI) re-grep-confirmed after the probe restore; ext directory test (PluginTest.php:224-256) green. |
| F-3 | low | **fixed** | Warning precedes `@unlink` in both paths (Plugin.php:159-160, :230-231); full source read this round. |
| F-4 | low (KB) | **still present in KB — accepted-pending** (main session MUST apply at step 14, before merge) | `decisions.md:96-100` read first-hand again: DEC-006 still swaps #185/#182 and the re-verification sentence goes stale on merge. Not re-counted as open. |
| F-5 | nit | **fixed** | `testExistingBinaryIsInvalidWhenFileMissing` confirmed in branch diff of tests/ChecksumManagerTest.php. |
| F-6 | nit | **fixed** | `### Fixed` at CHANGELOG.md:63-69 under [Unreleased], content accurate (branch diff re-read). |
| F-7 | low | **fixed** | Factory (PluginTest.php:325-336) roots each stub at its own directory; stub always throws (wrong-dir writes structurally impossible); `=== 1` per instance; green on this ffi-loaded machine. |
| F-8 | nit | **fixed** | Failing stub + `assertFileDoesNotExist` (:140, :179) + fallback messages + exactly-once; unlinks mutation-proven rounds 3/4. |
| F-9 | low | **fixed** | FFI mismatch test (PluginTest.php:145-182) ran (not skipped) and passed in focused run; FAQ-003 skip confirmed; FFI `@unlink` pinned (probe A, round 4). |
| F-10 | nit | **fixed** | Closure arity (PluginTest.php:329) matches invocation (:409) and docblock (:401); PHPStan `[OK] No errors` (53 files). |
| F-11 | low | **fixed** | New test `testPackageInstallWithDirectoryAtFfiTargetPathFailsCleanly` (PluginTest.php:184-222) plants a **directory** at the FFI target path and asserts: no 'failed SHA-256 verification' warning (:218), 'FFI library download failed' (:219), `$ffiDownloader->downloadCalls === 1` (:220), directory untouched (:221); FAQ-003 skip guard; ran (not skipped) on PHP 8.5.9 + ffi. **Mutation probe this round:** FFI `is_file()` → `file_exists()` (Plugin.php:221) → test **RED** at :218 (output contains the pre-fix F-2 failure-mode warning); restored from throwaway copy, `git diff --exit-code` and `git status --porcelain` clean, re-run green. The FFI is_file guard is genuinely pinned. |

New findings: **none** — no F-12. Full `git show 39a8b13` read (one test + review artifacts only) and branch-wide holistic sweep (docs, CHANGELOG, composer.json empty diff, fail-closed `BinaryDownloader` untouched, no debug artifacts, no FAQ-007 message-comparison additions) found nothing new. Acknowledged out-of-scope items re-confirmed, not re-counted: `InstallScript` dead-code accept path (coder Finding 3; unreferenced — `extra.class` → `Composer\Plugin`), `curl_close` 8.5 deprecation (coder Finding 1), naming duplication (coder Finding 2), message-equality catches (#184), post-install FFI trust (#65/#182).

Gates: `composer lint` pass (cs-fixer 0/55, PHPStan OK, rector OK, kb-lint 0); `composer test` pass — **5413 tests / 11573 assertions** (exactly +1 test / +4 assertions vs round 4 = the new FFI directory test), exit 0, deprecations identical to the pre-existing set (8 fgetcsv + 1 PHPUnit in BuilderTest from #57), 1 skip per FAQ-001; focused PluginTest 7 tests / 30 assertions OK, both FFI tests ran (not skipped); mutation probe fully rolled back — tree clean except the two review output files; composer.json diff empty; no gate changes proposed.

KB candidates (single-writer rule — main session applies at step 14): DEC-006 update re-confirmed with end-of-cycle urgency (F-4 — apply before merge); FAQ-010 validated by committed fix; FAQ-011 now validated **three times** — F-9 (planted file), F-11 (planted directory), and round 5's probe proving F-11's fix genuinely flips red on the `file_exists` regression. Full texts in review-5.md. Tag-index note re-confirmed: no FAQ/DEC entry has a `tests` tag — FAQ-010/011 should add it.
